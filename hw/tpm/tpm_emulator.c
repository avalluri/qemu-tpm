...
#define RESPONSE_BUFFER_SIZE(buf) \
    be32_to_cpu(((struct tpm_resp_hdr *)buf)->len)
...
static int tpm_emulator_check_caps(TPMEmulator *tpm_pt)
{
    ...
    case TPM_VERSION_UNSPEC:
        error_report("tpm-emulator: %s: TPM version has not been set", __func__);
        return -1;
    }
