                                     ...
                                     bool *selftest_done)
{
    ...
    if (!tpm_pt->child_running) return -1;
                                       ...
                                       gpointer user_data)
{
    ...
    TPMEmulator *tpm_pt = TPM_EMULATOR(thr_parms->tb);
    ...
    case TPM_BACKEND_CMD_PROCESS_CMD:
        locty = tpm_state->locty_data;
        if (tpm_emulator_set_locality(tpm_pt, tpm_state->locty_number) < 0) {
            tpm_write_fatal_error_response(locty->r_buffer.buffer,
                                           locty->r_buffer.size);
        } else {
...
static void tpm_emulator_shutdown(TPMEmulator *tpm_pt)
{
    ...
    if (!tpm_pt->child_running) return;

    if (tpm_util_ctrlcmd(tpm_pt->tpm_ctrl_fd, PTM_SHUTDOWN, &res, 0,
                         sizeof(res)) < 0) {
        error_report("tpm-eulator: Could not cleanly shutdown the TPM: %s",
                     strerror(errno));
    } else if (res != 0) {
...
static int tpm_emulator_probe_cabs(TPMEmulator *tpm_pt)
{
    if (!tpm_pt->child_running) return -1;
                             ...
                             TPMRecvDataCB *recv_data_cb)
{
    ...
    if (tpm_emulator_spawn_emulator(tpm_pt) < 0) {
        error_report("Failed to spawn emulator");
        return -1;
    }
...
static bool tpm_emulator_get_startup_error(TPMBackend *tb)
{
    ...
    DPRINTF("tpm_emulator: %s:%d", __func__, tpm_pt->child_running);
...
static int tpm_emulator_handle_device_opts(TPMEmulator *tpm_pt, QemuOpts *opts)
{
    ...
    if ((value = qemu_opt_get(opts, "logfile"))) {
